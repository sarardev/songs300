...
function allomani_array_path($array, $keys){
    foreach($keys as $key){
  ...
  $qr=db_query("select * from songs_banners where active=1 and ((pages like '%$pg_view,%' and (menu_pos='l' or menu_pos='r')) or (pages like '%$c_pg_view,%' and menu_pos='c')) and (start_date <= ".time()." or start_date=0) and (expire_date >= ".time()." or expire_date=0) order by `type`,ord,menu_pos"); 
    ...
    //       cache_set("banners:$pg_view:$c_pg_view",$banners);
      ...
      //   }
        ...
        unset($qr,$data);
       //----------------------- Left Content --------------------------------------------
      if(count(allomani_array_path($blocks, array('l')))){
        print "<td width='$blocks_width' valign=\"top\" align=\"center\" dir=\"$global_dir\">
        <table width=100%>" ;

        $adv_c = 1 ;
         foreach(allomani_array_path($blocks, array('l',0)) as $xdata){

        print "<tr>
                <td  width=\"100%\" valign=\"top\">";
                
   //     $sub_qr = db_query("select * from songs_blocks where active=1 and cat='$xdata[id]' and pages like '%$pg_view,%' order by ord");
          $left_sub_blocks = allomani_array_path($blocks, array('l',$xdata['id']));
          $sub_count = count($left_sub_blocks);
        ...
        $tabs->start($xdata['title']);
          ...
          foreach($left_sub_blocks as $sub_data){
        ...
        $left_menu_banners = allomani_array_path($banners, array('menu','l',$adv_c));
        if(count($left_menu_banners)){
       print_block_banners($left_menu_banners);  
...
if(count(allomani_array_path($banners, array('menu','l')))){
    foreach(allomani_array_path($banners, array('menu','l')) as $data_array){
...
//---------------------  Header Banners ----------------------------
 ...
 $header_banners = allomani_array_path($banners, array('header','x',0));
 if(count($header_banners)){
  
 foreach($header_banners as $data){
...
//-------------------------- CENTER CONTENT ---------------------------------------------
     ...
     //--------- open banners ----------//
    $bnx = 0 ;
   $open_banners = allomani_array_path($banners, array('open','x',0));
   if(count($open_banners)){
 foreach($open_banners as $data){
   ...
   print "<script>
   function pop_close(){";
    $close_banners = allomani_array_path($banners, array('close','x',0));
    if(count($close_banners)){ 
   foreach($close_banners as $data){   
      db_query("update songs_banners set views=views+1 where id='$data[id]'");
...
//---------------------------
 ...
 $adv_c = 1 ;
  ...
  if(count(allomani_array_path($blocks, array('c',0)))){
         foreach(allomani_array_path($blocks, array('c',0)) as $ydata){

    
           $center_sub_blocks = allomani_array_path($blocks, array('c',$ydata['id']));
           $sub_count = count($center_sub_blocks);    
        ...
        $tabs->start($ydata['title']);
          ...
          foreach($center_sub_blocks as $sub_data){    
      ...
      //----------------- Center Menus Banners-----------------------
       ...
       $center_menu_banners = allomani_array_path($banners, array('menu','c',$adv_c));
       if(count($center_menu_banners)){
              print_block_banners($center_menu_banners,"center");  
       ...
       ++$adv_c ; 
        ...
        //------- print remaining blocks banners --------//
if(count(allomani_array_path($banners, array('menu','c')))){
    foreach(allomani_array_path($banners, array('menu','c')) as $data_array){
