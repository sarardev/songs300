...
class sec_img_verification {
   ...
   var $height = 100 ;
   var $width = 30 ;
	function __construct ($height = 150, $width = 35, $sid = NULL) {
		$this->sec_img_verification($height, $width, $sid);
	}
	...
	function verify_string ($gd_string) {
		$session_string = isset($_SESSION['gd_string']) ? (string) $_SESSION['gd_string'] : '';
		$gd_string = (string) $gd_string;

		// Check if the original string and the passed string match...
		if ($session_string !== '' && strtolower($session_string) === strtolower($gd_string)) {
              $_SESSION['gd_string'] = ''; 
			return TRUE;
		} else {
			return FALSE;
