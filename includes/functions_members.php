...
function member_prepare_birth_date($year, $month, $day){
global $members_connector;
...
if($year !== '' && $month !== '' && $day !== ''){
    $year_i = intval($year);
    $month_i = intval($month);
    $day_i = intval($day);

    if($year_i > 0 && $month_i > 0 && $day_i > 0 && checkdate($month_i, $day_i, $year_i)){
        $birth_date = sprintf('%04d-%02d-%02d', $year_i, $month_i, $day_i);
        return connector_get_date($birth_date,'member_birth_date');
    }
}

if(!empty($members_connector['enable'])){
    return '';
}
