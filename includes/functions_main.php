...
function print_videos_path_links($cat,$filename=""){
   ...
   $data_cat = db_qr_fetch("select name,id,cat,path from songs_videos_cats where id='$cat'"); 
     if(is_array($data_cat) && !empty($data_cat['path'])){         
   ...
   if($id){
      $dir_data = isset($cats_data[$id]) ? $cats_data[$id] : array();

        if(!empty($dir_data['id']) && isset($dir_data['name'])){
...
if((is_array($data_prev) && !empty($data_prev['id'])) || (is_array($data_next) && !empty($data_next['id']))){ 
