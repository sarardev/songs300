...
class InputFilter {
	...
	function __construct($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1) {
		$this->inputFilter($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
	}
	...
	function decode($source) {
		...
		// convert decimal
		$source = preg_replace_callback('/&#(\d+);/m', function($matches) { return chr($matches[1]); }, $source);				// decimal notation
		// convert hex
		$source = preg_replace_callback('/&#x([a-f0-9]+);/mi', function($matches) { return chr(hexdec($matches[1])); }, $source);	// hex notation
		return $source;
	}
