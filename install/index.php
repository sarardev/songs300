<?php
    ...
    require_once(CWD ."/includes/mysql_compat.php");
    ...
    $request_db_host = isset($_REQUEST['db_host']) ? $_REQUEST['db_host'] : '';
    $request_db_name = isset($_REQUEST['db_name']) ? $_REQUEST['db_name'] : '';
    $request_db_user = isset($_REQUEST['db_user']) ? $_REQUEST['db_user'] : '';
    $request_db_pass = isset($_REQUEST['db_pass']) ? $_REQUEST['db_pass'] : '';
    $request_db_force_clean = !empty($_REQUEST['db_force_clean']);
    $request_db_version = isset($_REQUEST['db_version']) ? $_REQUEST['db_version'] : '';
...
<?php
    ...
    function verify_step_2(){
        global $msg,$db_host,$db_username,$db_password,$db_name,$request_db_host,$request_db_name,$request_db_user,$request_db_pass;

        //---- write new config ----//
        $config_file = file_get_contents(CWD ."/config.php");
        $config_file = preg_replace("/".'\$'."db_host[\s+]?=[\s+]?\"(.*)\"[\s+]?;/","\$db_host = \"".addslashes($request_db_host)."\";",$config_file);
        $config_file = preg_replace("/".'\$'."db_name[\s+]?=[\s+]?\"(.*)\"[\s+]?;/","\$db_name = \"".addslashes($request_db_name)."\";",$config_file);
        $config_file = preg_replace("/".'\$'."db_username[\s+]?=[\s+]?\"(.*)\"[\s+]?;/","\$db_username = \"".addslashes($request_db_user)."\";",$config_file);
        $config_file = preg_replace("/".'\$'."db_password[\s+]?=[\s+]?\"(.*)\"[\s+]?;/","\$db_password = \"".addslashes($request_db_pass)."\";",$config_file);
    ...
    function print_step_2(){
        ...
        <table width=100%>
        <tr><td>Database Host</td><td><input type=text name='db_host' value=\"".htmlspecialchars(isset($db_host[1]) ? $db_host[1] : '')."\"></td></tr>
        <tr><td>Database Username</td><td><input type=text name='db_user' value=\"".htmlspecialchars(isset($db_user[1]) ? $db_user[1] : '')."\"></td></tr>
        <tr><td>Database Password</td><td><input type=text name='db_pass' value=\"".htmlspecialchars(isset($db_pass[1]) ? $db_pass[1] : '')."\"></td></tr>
        <tr><td>Database Name</td><td><input type=text name='db_name' value=\"".htmlspecialchars(isset($db_name[1]) ? $db_name[1] : '')."\"></td></tr>
        </table>
    ...
    function verify_step_3(){
        global $msg,$request_db_force_clean;
    ...
    function verify_step_4(){
        ...
        global $msg,$db_versions,$request_db_version;

        require (CWD . "/config.php");
                                  
    if(empty($request_db_version) || !isset($db_versions[$request_db_version])){
     $msg = "Please select database version";
     return false;
    }

    $newImport = new sqlImport ($db_host, $db_username, $db_password,$db_name, CWD."/install/".$request_db_version);
$newImport -> import ();
