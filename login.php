...
require("global.php") ;
   ...
   $md5pwd = isset($md5pwd) ? $md5pwd : '';
   $md5pwd_utf = isset($md5pwd_utf) ? $md5pwd_utf : '';
...
if(db_num($qr)){
   ...
   $data = db_fetch($qr);
      ...
      if(member_verify_password($data[members_fields_replace('id')],$password,$md5pwd,$md5pwd_utf)){
        $fresh_password_data = db_qr_fetch("select ".members_fields_replace('password')." as password from ".members_table_replace("songs_members")." where ".members_fields_replace("id")."='".$data[members_fields_replace('id')]."'",MEMBER_SQL);
        if(!empty($fresh_password_data['password'])){
            $data[members_fields_replace('password')] = $fresh_password_data['password'];
        }
