...
//---------------------------------------------------------
$year = (int) $year;
print "<div class='albums-page'><div class='albums-page-shell'>";

$qr = db_query("select year from songs_albums where year > 0 group by year order by year desc");
if(db_num($qr)){
print "<section class='albums-content-card albums-filter-card'>";
open_table();                                     
...
$page_result = db_qr_fetch("select count(*) as count from songs_albums ".iif($year," where year like '$year'"));
 ...
 print "<section class='albums-content-card albums-list-card'>";
 open_table();   
