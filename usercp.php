...
db_query("update ".members_table_replace('songs_members')." set field_".$m_custom_id."='".db_escape($m_custom_name)."' where ".members_fields_replace('id')."='$member_data[id]'",MEMBER_SQL);
        ...
        close_table();
          ...
          $safe_member_username = h($data[members_fields_replace('username')]);
          $safe_member_email = h_attr($data[members_fields_replace('email')]);
...
<img src='$style[images]/profile_preview.gif' border=0>&nbsp; $phrases[profile_preview] </a>
         ...
         if($settings['members_profile_pictures']){     
          ...
          <td width=20%>
         $phrases[username] :
          </td><td>".$safe_member_username."</td>  </tr>
           <td width=20%>
          $phrases[email] :
          </td><td ><input type=text name=email value='".$safe_member_email."' size=30></td>  </tr>
          </tr></table>
          </fieldset>
          <br>
         <fieldset style=\"padding: 2\">
