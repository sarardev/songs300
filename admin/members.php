...
$birth_date = member_prepare_birth_date($date_y, $date_m, $date_d);
 ...
 $insert_result = db_query("insert into ".members_table_replace('songs_members')." (".members_fields_replace('username').",".
 members_fields_replace('email').",".members_fields_replace('country').",".members_fields_replace('birth').",".
 members_fields_replace('usr_group').",".members_fields_replace('date').",gender,active_code,img,thumb,pm_email_notify,privacy_settings,members_list)
 values('$username','$email','".db_escape($country)."','".db_escape($birth_date,false)."','$usr_group','".connector_get_date(time(),'member_reg_date')."','".db_escape($gender)."','','','','1','".db_escape($default_privacy_settings,false)."','1')",MEMBER_SQL);


 $member_id = $insert_result ? mysql_insert_id() : 0;
 if(!$member_id && $insert_result){
 $inserted_member = db_qr_fetch("select ".members_fields_replace('id')." as id from ".members_table_replace('songs_members')." where ".members_fields_replace('email')."='$email' and ".members_fields_replace('username')."='$username' order by ".members_fields_replace('id')." desc", MEMBER_SQL);
 $member_id = isset($inserted_member['id']) ? intval($inserted_member['id']) : 0;
 }

if(!$member_id){
 print "<center><table width=50% class=grid><tr><td align=center>$phrases[err]</td></tr></table></center><br>";
}else{
//------------- Custom Fields  ------------------
   if(is_array($custom) && is_array($custom_id)){
