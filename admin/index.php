...
ob_start(function($content){
    return allomani_inject_csrf_into_forms($content,'admin');
});
...
$cp_username = isset($cp_username) ? $cp_username : '';
$cp_password = isset($cp_password) ? $cp_password : '';
$cp_email = isset($cp_email) ? $cp_email : '';

if($_SERVER['REQUEST_METHOD'] === 'POST'){
    if(!allomani_verify_csrf_request('admin')){
        print "<br><center><table width=60% class=grid><tr><td align=center>Security token invalid or expired. Please refresh the page and try again.</td></tr></table></center>";
        exit();
    }
}
...
if ($action == "login" && $username && $password ){
     ...
     $login_data=db_fetch($result);
       ...
       if(allomani_password_verify($password,$login_data['password'])){
 if(allomani_password_needs_rehash($login_data['password'])){
     $new_admin_hash = allomani_password_hash($password);
     db_query("update songs_user set password='".db_escape($new_admin_hash,false)."' where id='".intval($login_data['id'])."'");
     $login_data['password'] = $new_admin_hash;
 }
 access_log_record($login_data['username'],"Login Done");
 
admin_session_login($login_data);
...
$main_user_password = allomani_password_hash($cp_password);
$insert_user = db_query("insert into songs_user (username,password,email,group_id,permisions,cp_permisions) values('".db_clean_string($cp_username,"code")."','".db_escape($main_user_password,false)."','".db_escape($cp_email)."','1','','')");
if($insert_user){
print "<SCRIPT>window.location=\"index.php\";</script>";
exit();
}else{
$main_user_error = mysql_error();
}
}else{
$main_user_error = "Please enter username and password";
}
}
//-------- First time setup ----------//
$users_num = db_qr_fetch("select count(id) as count from songs_user");
...
";
if($main_user_error){
print "<tr><td colspan=2><center><font color=red>$main_user_error</font></center></td></tr>";
}
print "
...
db_query("delete from songs_user where id='$id'");
        ...
        $new_user_password = allomani_password_hash($password);
        db_query("insert into songs_user (username,password,email,group_id,permisions,cp_permisions) values ('".db_escape($username,false)."','".db_escape($new_user_password,false)."','".db_escape($email)."','".intval($group_id)."','','')");
        }
        }else{
        ...
        if ($action == "edituserok"){
                if ($password){
                $hashed_edit_password = allomani_password_hash($password);
                $ifeditpassword = ", password='".db_escape($hashed_edit_password,false)."'" ;
                }
...
//----------------------------------------------------
  ...
  print " <center> <table cellpadding=\"0\" border=0 cellspacing=\"0\" width=\"80%\" class=\"grid\">
      ...
      while($data = db_fetch($result)){
        $safe_user_id = intval($data['id']);
        $safe_username = h($data['username']);
        $safe_email = h($data['email']);


        if ($data['group_id']==1){$groupname="$phrases[cp_user_admin]";
             $permision_link="";
      }elseif($data['group_id']==2){$groupname="$phrases[cp_user_mod]";
       $permision_link="<a href='index.php?action=permisions&id=$safe_user_id'>$phrases[permissions_manage]</a>";
        ...
        print "<tr>
                <td  width=\"134\" >
                <p align=\"center\">$safe_username</p></td>
                <td  width=\"240\" >
                <p align=\"center\">$safe_email</p></td>
                <td  width=\"105\"><p align=\"center\">$groupname</p></td>
                 <td  width=\"105\"><p align=\"center\">$permision_link</p></td>
                <td  width=\"193\"><p align=\"center\">
                 <a href='index.php?action=edituser&id=$safe_user_id'> $phrases[edit] </a> ";
        if ($data['id'] !="1"){
                print "- <a href='index.php?action=deluser&id=$safe_user_id' onClick=\"return confirm('".$phrases['are_you_sure']."');\"> $phrases[delete] </a>";
        }
                print " </p>
                </td>
        </tr>";
...
$data = db_fetch($qr) ;
$safe_edit_id = intval($data['id']);
$safe_edit_username = h_attr($data['username']);
$safe_edit_email = h_attr($data['email']);
$safe_edit_username_text = h($data['username']);

print "<img src='images/arrw.gif'>&nbsp;<a href='index.php?action=users'>$phrases[the_users]</a> / $safe_edit_username_text <br><br>
...
<form method=\"post\" action=\"index.php\">
 ...
 <TABLE width=70% class=grid>
    ...
    <INPUT TYPE=\"hidden\" NAME=\"id\" \" value=\"$safe_edit_id\" >
<INPUT TYPE=\"hidden\" NAME=\"action\"  value=\"edituserok\" >

   <TD width=\"100\"><b>$phrases[cp_username] : </b></TD>
   <TD width=\"614\"><INPUT TYPE=\"text\" NAME=\"username\" size=\"32\" value=\"$safe_edit_username\" > </TD>
  </TR>
    <TR>
...
<div id=\"passwordStrength\" class=\"strength0\"></div>
   ...
   <TD width=\"614\"><INPUT TYPE=\"text\" NAME=\"email\" size=\"32\" value=\"$safe_edit_email\" > </TD>
  </TR>";
...
$disable_auto_admin_redirect = isset($disable_auto_admin_redirect) ? $disable_auto_admin_redirect : true;
$admin_http_host = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '';
$admin_request_uri = isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '';

if(!$disable_auto_admin_redirect){
if($admin_http_host && strchr($admin_http_host,"www.")){
  print "<SCRIPT>window.location=\"http://".str_replace("www.","",$admin_http_host).$admin_request_uri."\";</script>";
  die();
  }
 }
...
if(defined('COPYRIGHTS_TXT_ADMIN_LOGIN') && COPYRIGHTS_TXT_ADMIN_LOGIN){
if($global_lang=="arabic"){
    print "<br>
                <center>
<table width=60% class=grid><tr><td align=center>
