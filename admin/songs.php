 ...
 */
 ...
 if($action=="singer_add"){
  if_cat_admin($cat);
       ...
       $cat_name = !empty($data_cat['name']) ? $data_cat['name'] : '';
    print "<img border=0 src='images/arrw.gif'><a href='index.php?action=singers'> $phrases[the_songs_and_singers] </a> / <a href='index.php?action=singers&cat=$cat'> $cat_name</a> / $phrases[singer_add]<br><br>"; 
...
$qr_cat=db_query("select id,name from songs_cats where id='$cat' order by ord asc");
  ...
  if_cat_admin($cat);
  $data_cat = db_qr_fetch("select id,name from songs_cats where id='$cat'");
