...
//-----------------------------
if(!defined("MEMBER_SQL")){define("MEMBER_SQL","member_sql");}
//------------------------------
if(!defined("GLOBAL_LOADED")){define("GLOBAL_LOADED",true);}
//----------------------------------
if(!defined("SCRIPT_NAME")){define("SCRIPT_NAME","songs");}
if(!defined("SCRIPT_VER")){define("SCRIPT_VER","3.0");}
if(!defined("COPYRIGHTS_TXT_MAIN")){define("COPYRIGHTS_TXT_MAIN",true);}
if(!defined("COPYRIGHTS_TXT_ADMIN")){define("COPYRIGHTS_TXT_ADMIN",true);}
if(!defined("COPYRIGHTS_TXT_ADMIN_LOGIN")){define("COPYRIGHTS_TXT_ADMIN_LOGIN",true);}
//---------------------------------------------
//----------- current work dir definition -------
if(!defined('CWD')){define('CWD', (($getcwd = getcwd()) ? str_replace("\\","/",$getcwd) : '.'));}
//define('CFN',str_replace(CWD."/",'',$_SERVER['SCRIPT_FILENAME']));
if(!defined('CFN')){define('CFN',basename($_SERVER['SCRIPT_FILENAME']));}
//---------------------------------------------
...
//------ clean global vars ---------//
$_SERVER['QUERY_STRING'] = isset($_SERVER['QUERY_STRING']) ? strip_tags($_SERVER['QUERY_STRING']) : '';
$_SERVER['PHP_SELF'] = isset($_SERVER['PHP_SELF']) ? strip_tags($_SERVER['PHP_SELF']) : '';
$_SERVER['REQUEST_URI'] = isset($_SERVER['REQUEST_URI']) ? strip_tags($_SERVER['REQUEST_URI']) : '';
$_SERVER['HTTP_HOST'] = isset($_SERVER['HTTP_HOST']) ? strip_tags($_SERVER['HTTP_HOST']) : 'localhost';
$_SERVER['REMOTE_ADDR'] = isset($_SERVER['REMOTE_ADDR']) ? strip_tags($_SERVER['REMOTE_ADDR']) : '';

if(!defined('REQUEST_URI')){define('REQUEST_URI', 'REQUEST_URI');}
if(!defined('PHP_SELF')){define('PHP_SELF', 'PHP_SELF');}
if(!defined('HTTP_HOST')){define('HTTP_HOST', 'HTTP_HOST');}
if(!defined('REMOTE_ADDR')){define('REMOTE_ADDR', 'REMOTE_ADDR');}
if(!defined('ALLOMANI_SESSION_NAME')){define('ALLOMANI_SESSION_NAME', 'allomani_admin_sid');}

define("CUR_FILENAME",$_SERVER['PHP_SELF']);

function allomani_is_https(){
...
if(function_exists('session_status') && session_status() === PHP_SESSION_NONE){
    $session_secure = allomani_is_https();
    if(PHP_VERSION_ID >= 70300){
        ...
        @session_set_cookie_params(0, '/; samesite=Lax', '', $session_secure, true);
    }
    session_name(ALLOMANI_SESSION_NAME);
    @session_start();
}

//---------------------- common variables types clean-----------------------
 if(isset($id) && $id){if(is_array($id)){$id=array_map("intval",$id);}else{$id=(int) $id;}}
if(isset($cat) && $cat){if(is_array($cat)){$cat=array_map("intval",$cat);}else{$cat=(int) $cat;}}
//-------------------------------------------------------------------
...
$document_root = isset($_SERVER['DOCUMENT_ROOT']) ? $_SERVER['DOCUMENT_ROOT'] : dirname(CWD);

if(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] && strtolower($_SERVER['HTTPS']) != 'off'){
$siteurl = "https://$http_host" ;
}else{
$siteurl = "http://$http_host" ;
}

if(!isset($script_path)){
$script_path = trim(str_replace(rtrim(str_replace('\\', '/',$document_root),"/"),"",CWD),"/");
}
$scripturl = $siteurl . iif($script_path,"/".$script_path,"");
$upload_types = explode(',',str_replace(" ","",$settings['uploader_types']));
$mailing_email = str_replace("{domain_name}",$http_host,$settings['mailing_email']);
...
{
global $settings,$phrases,$style,$scripturl,$siteurl,$global_dir,$global_align,$global_align_x;
$arr = array();
...
$user_info = array();
function admin_session_login($login_data){
    if(function_exists('session_regenerate_id')){
        @session_regenerate_id(true);
    }
    $_SESSION['admin_user'] = array(
        'id' => intval($login_data['id']),
        'username' => $login_data['username'],
        'password_token' => md5($login_data['password']),
    );
}

function admin_session_logout(){
...
function allomani_get_csrf_token($context='default'){
    if(!isset($_SESSION['allomani_csrf']) || !is_array($_SESSION['allomani_csrf'])){
        $_SESSION['allomani_csrf'] = array();
    }
    if(empty($_SESSION['allomani_csrf'][$context])){
        $_SESSION['allomani_csrf'][$context] = bin2hex(random_bytes(32));
    }
    return $_SESSION['allomani_csrf'][$context];
}

function allomani_csrf_input($context='default'){
    $token = allomani_get_csrf_token($context);
    return '<input type="hidden" name="csrf_token" value="'.$token.'">';
}

function allomani_verify_csrf_token($token,$context='default'){
    if(!is_string($token) || $token === ''){
        return false;
    }
    $session_token = allomani_get_csrf_token($context);
    return hash_equals($session_token, $token);
}

function allomani_verify_csrf_request($context='default'){
    $request_token = isset($_POST['csrf_token']) ? $_POST['csrf_token'] : '';
    return allomani_verify_csrf_token($request_token,$context);
}

function allomani_inject_csrf_into_forms($content,$context='default'){
    return preg_replace_callback('/<form\b[^>]*>/i', function($matches) use ($context){
        $form_tag = $matches[0];
        if(!preg_match('/\bmethod\s*=\s*([\"\'])?post\1?/i', $form_tag)){
            return $form_tag;
        }
        return $form_tag . allomani_csrf_input($context);
    }, $content);
}

function allomani_password_hash($password){
    return password_hash($password, PASSWORD_DEFAULT);
}

function allomani_password_is_modern_hash($stored_hash){
    if(!is_string($stored_hash) || $stored_hash === ''){
        return false;
    }
    $password_info = password_get_info($stored_hash);
    return !empty($password_info['algo']) && !empty($password_info['algoName']) && $password_info['algoName'] !== 'unknown';
}

function allomani_password_verify($password,$stored_hash){
    if(!is_string($stored_hash) || $stored_hash === ''){
        return false;
    }
    if(allomani_password_is_modern_hash($stored_hash)){
        return password_verify($password,$stored_hash);
    }
    if(hash_equals($stored_hash, md5($password))){
        return true;
    }
    return hash_equals($stored_hash, $password);
}

function allomani_password_needs_rehash($stored_hash){
    if(!is_string($stored_hash) || $stored_hash === ''){
        return true;
    }
    if(!allomani_password_is_modern_hash($stored_hash)){
...
$session_user = isset($_SESSION['admin_user']) && is_array($_SESSION['admin_user']) ? $_SESSION['admin_user'] : array();

if(!empty($session_user['id'])){
    $user_info['username'] = isset($session_user['username']) ? $session_user['username'] : '';
    $user_info['password'] = isset($session_user['password_token']) ? $session_user['password_token'] : '';
    $user_info['id'] = intval($session_user['id']);
}else{
...
function send_email($from_name,$from_email,$to_email,$subject,$msg,$html=0,$encoding=""){
        global $smtp_settings,$settings,$script_path;
...
if(!$encoding){$encoding =  $settings['site_pages_encoding'];}
 ...
 $subject = "=?".$encoding."?B?".base64_encode($subject)."?=";     
    ...
    $mailHeader .= "X-Mailer-File: "."http://".$_SERVER['HTTP_HOST'].($script_path ? "/".$script_path:"").$_SERVER['PHP_SELF']."\r\n";
...
$hooklocations = array();
    ...
    while (($file = readdir($handle)) !== false)
    {
        ...
        $xmlobj = new Allomani_XMLparser(false, CWD . "/xml/$file");
        $xml = $xmlobj->parse();
...
if(PHP_VERSION_ID >= 70300){
    ...
    setcookie($name, $value, $k_timeout, $cookies_path . '; samesite=Lax', $cookies_domain, $secure, true);
}
}
//--------- get cookies funtion ---------
...
function allomani_array_replace($tofind, $toreplace,$a){
...
function get_players($id=""){
    ...
    while($data=db_fetch($qr)){
        $players[$i] = $data;
        $extensions = explode(",",strtolower($data['exts']));
        $players[$i]['exts'] = array();
...
if(!count($players)){
    ...
    if(count($players)){
        ...
        foreach($players as $player){
            if($player['id'] != 1 && is_array($player['exts']) && $extension){
...
function run_player($url,$handler='int_content',$players=array()){
global $scripturl,$settings,$phrases,$data;

if(!$handler){$handler='int_content';}
$player_data = get_player_data($url, $players);
